...
- Prefer the helper over hand-rolled `git`, direct Grok Build CLI strings, or any other Bash activity.
...
- If the forwarded request includes `--model`, normalize `build` to `grok-build` and pass it through to `task`.
